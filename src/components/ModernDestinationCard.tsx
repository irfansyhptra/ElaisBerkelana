...
const ModernDestinationCard = ({
  ...
  // Get dynamic backgrounds based on country
  ...
  // Helper functions to prevent hydration mismatch
  const getSafeBackgroundImage = () => {
    return isClient ? backgroundImage : "/images/destinations/indo.jpg";
  };

  const getSafeGradientClass = () => {
    if (!isClient) return "absolute inset-0 bg-gradient-to-br from-emerald-900/70 via-blue-900/50 to-purple-900/60";
    return `absolute inset-0 bg-gradient-to-br ${gradientOverlay}`;
  };

  const getSafeAccentClass = (baseClasses: string) => {
    if (!isClient) return `bg-gradient-to-r from-yellow-400 via-orange-500 to-red-500 ${baseClasses}`;
    return `bg-gradient-to-r ${accentColor} ${baseClasses}`;
  };

  const getSafeAccentClassBr = (baseClasses: string) => {
    if (!isClient) return `bg-gradient-to-br from-yellow-400 to-orange-500 ${baseClasses}`;
    return `bg-gradient-to-br ${accentColor} ${baseClasses}`;
  };

  // Featured card variant - lebih besar dan mencolok
  if (variant === "featured") {
    return (
      <div className="group col-span-full md:col-span-2 transform transition-all duration-500 hover:-translate-y-4 hover:scale-105">
        <Link href={href}>
          <div className="relative h-96 rounded-3xl overflow-hidden shadow-3xl hover:shadow-4xl transition-all duration-700 transform-gpu">
            {/* Main background */}
            ...
            <div className="absolute top-6 left-6 space-y-2">
              <div
                className={`w-12 h-12 bg-gradient-to-br ${accentColor} rounded-2xl flex items-center justify-center shadow-lg`}
              >
                <span className="text-white text-xl">⭐</span>
              </div>
              <div
                className={`w-2 h-16 bg-gradient-to-b ${accentColor} rounded-full opacity-60`}
              ></div>
            </div>
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-8">
              ...
              <div className="flex items-center space-x-4">
                <div
                  className={`flex-1 h-2 bg-gradient-to-r ${accentColor} rounded-full transform scale-x-0 group-hover:scale-x-100 transition-transform duration-700 origin-left`}
                ></div>
            ...
            <div className="absolute inset-0 opacity-10">
              <div
                className="absolute top-1/4 left-1/4 w-32 h-32 border border-white/30 rounded-full animate-spin"
                style={{ animationDuration: "20s" }}
              ></div>
              <div
                className="absolute bottom-1/4 right-1/4 w-24 h-24 border border-white/20 rounded-full animate-spin"
                style={{
  ...
  return (
    ...
    >
      <Link href={href}>
        <div className="relative h-80 rounded-2xl overflow-hidden shadow-2xl hover:shadow-3xl transition-all duration-700 transform-gpu glow-effect">
          {/* Background dinamis berdasarkan negara */}
          <div className="absolute inset-0">
            <OptimizedImage
              src={backgroundImage}
            ...
            />
            {/* Sophisticated gradients berdasarkan negara */}
            <div
              className={`absolute inset-0 bg-gradient-to-br ${gradientOverlay}`}
            ></div>
          ...
          <div
            className={`absolute top-6 left-6 w-4 h-4 bg-gradient-to-br ${accentColor} rounded-full animate-pulse`}
          ></div>
          ...
          <div className="absolute inset-0 flex flex-col justify-end p-6">
            ...
            <div className="flex items-center justify-between">
              <div
                className={`flex-1 h-1 bg-gradient-to-r ${accentColor} rounded-full transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left`}
              ></div>
          ...
          <div className="absolute inset-0 opacity-5 group-hover:opacity-10 transition-opacity duration-500">
            <div
              className="absolute top-1/3 left-1/3 w-20 h-20 border border-white/30 rounded-full animate-spin"
              style={{ animationDuration: "20s" }}
            ></div>
            <div
              className="absolute bottom-1/3 right-1/3 w-16 h-16 border border-white/20 rounded-full animate-spin"
              style={{
